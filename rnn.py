...
class RNN(object):
    ...
    def __init__(self, hidden_size, seq_length, learning_rate, data):
        ...
        self.p = 0                                                          # Variable to keep track of where we are in the data-file
    ...
    def train(self):
        ...
        if self.p + self.seq_length+1 >= len(self.data) or self.iter == 0: 
        ...
        if self.iter % 1000 == 0:
            ...
            txt = self.sampleToString(sample_ix)
    ...
    def sampleToString(self, sample_ix):
        """Takes a sample and makes it to a string."""
        txt = ''.join(self.ix_to_char[ix] for ix in sample_ix)
    ...
    def iterationAndLoss(self):
        """Returns a string representing the iteration number and loss.
         - iter 17000, loss: 12.522321 
        """
        return 'iter %d, loss: %f' % (self.global_iter, self.smooth_loss)
